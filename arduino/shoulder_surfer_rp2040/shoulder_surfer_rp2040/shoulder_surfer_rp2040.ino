// ═══════════════════════════════════════════════════════════════
//  Shoulder Guardian — RP2040-Zero + HLK-LD2450 + GC9A01
//
//  RP2040-Zero variant.  For the ATmega32U4 or ESP32-C3 versions
//  see the sibling sketch folders.  All three speak the same
//  STATUS / SET-command protocol — the same Electron app works
//  with any of them.
//
//  ── Wiring — HLK-LD2450 radar ───────────────────────────────
//    LD2450 VCC  → 3.3V   (3.3 V — no voltage divider needed)
//    LD2450 GND  → GND
//    LD2450 TX   → GP 1   (Serial1 RX — UART0)
//    LD2450 RX   → GP 0   (Serial1 TX — UART0)
//
//  ── Wiring — GC9A01 1.28" round display ─────────────────────
//    GC9A01 VCC  → 3.3V
//    GC9A01 GND  → GND
//    GC9A01 SCL  → GP 2   (SPI0 SCK)
//    GC9A01 SDA  → GP 3   (SPI0 MOSI)
//    GC9A01 CS   → GP 5   (TFT_CS)
//    GC9A01 DC   → GP 4   (TFT_DC)
//    GC9A01 RST  → GP 6   (TFT_RST)
//    GC9A01 BLK  → GP 7   (TFT_BLK) or wire directly to 3.3V
//
//  ── Arduino IDE setup ────────────────────────────────────────
//    Board Manager : "Raspberry Pi Pico/RP2040" by Earle Philhower
//    Board         : "Waveshare RP2040 Zero"   (or RP2040 Zero)
//    USB Stack     : "Pico SDK"
//    Library       : "Arduino_GFX_Library" by Moon On Our Nation
//
//  Note: RP2040-Zero has a WS2812 RGB LED on GP16, not a plain
//  LED. Status indication uses a simple GP25 output by default —
//  wire an LED + 330Ω resistor there, or comment out the LED
//  calls and use the NeoPixel library if you prefer colour status.
// ═══════════════════════════════════════════════════════════════
...
// ── Pins ──────────────────────────────────────────────────────
// Status LED — wire a plain LED (+ 330 Ω) to this pin, or set
// to an unused pin and ignore.  GP25 is not broken out on the
// Zero but is a safe no-op if left floating.
#define LED_PIN       25
#define LED_ACTIVE_LOW false

// LD2450 UART0 (Serial1)
#define LD_RX          1   // GP1 — from LD2450 TX
#define LD_TX          0   // GP0 — to LD2450 RX
...
#define TFT_DC         4
#define TFT_RST        6
#define TFT_BLK        7   // HIGH = backlight on; wire to 3.3V to skip GPIO control
...
#define C_BG     0x0882   // #0d1117 — matches Electron app background
...
void setup() {
  ...
  // RP2040: assign specific GPIO pins before begin()
  Serial1.setRX(LD_RX);
  Serial1.setTX(LD_TX);
...
// ── Display ───────────────────────────────────────────────────
// mmToPx: radar mm coords → display pixel coords.
...
// The screen never shows a partially-updated state — no flicker, no tearing.
void drawRadar() {
  // ── Background ──────────────────────────────────────────────
  ...
  // Mask the strip below the sensor row (hides arc fragments)
