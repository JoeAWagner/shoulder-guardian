...
//
//  Protocol:
//    LD2450 → Arduino : 256000 baud binary frames (30 bytes each)
...
uint16_t maxRangeMm   = 2000;  // ignore targets beyond this (mm)
...
void setup() {
  ...
  Serial1.begin(256000);  // LD2450 binary frames
...
void parseFrame() {
  ...
  for (int i = 0; i < 3; i++) {
    int base = 4 + i * 8;
    targets[i].x     = (int16_t)(frameBuf[base]     | (frameBuf[base+1] << 8));
    targets[i].y     = (int16_t)(frameBuf[base+2]   | (frameBuf[base+3] << 8));
    targets[i].speed = (int16_t)(frameBuf[base+4]   | (frameBuf[base+5] << 8));

    // Valid = present AND within configured range
...
// ── Serial commands from app ──────────────────────────────────
void handleSerial() {
  if (!Serial.available()) return;
  String cmd = Serial.readStringUntil('\n');
  cmd.trim();

  if (cmd.startsWith("SET COOL ")) {
    uint16_t v = cmd.substring(9).toInt();
    if (v >= 1 && v <= 300) {
      cooldownSec = v;
      EEPROM.put(ADDR_COOL, cooldownSec);
      Serial.println("OK:COOL=" + String(v));
    }
  } else if (cmd.startsWith("SET LOCKDLY ")) {
    uint16_t v = cmd.substring(12).toInt();
    if (v >= 5 && v <= 120) {
      lockDelaySec = v;
      EEPROM.put(ADDR_LOCKDLY, lockDelaySec);
      Serial.println("OK:LOCKDLY=" + String(v));
    }
  } else if (cmd.startsWith("SET MAXRANGE ")) {
    uint16_t v = cmd.substring(13).toInt();
    if (v >= 500 && v <= 5000) {
      maxRangeMm = v;
      EEPROM.put(ADDR_MAXRANGE, maxRangeMm);
      Serial.println("OK:MAXRANGE=" + String(v));
    }
  } else if (cmd.startsWith("SET LOCKEN ")) {
    lockEnabled = cmd.substring(11).toInt() != 0;
